...
function checkAuth() {
    // With sessions, we check auth by attempting to load the gallery or a dedicated profile endpoint.
    // If the gallery load fails with 401, we know we're logged out.
    // For the UI state, we'll assume logged in if we don't get a 401.
    loadGallery();
}
...
function logout() {
    // Session logout would ideally hit a /logout endpoint
    location.reload();
}
